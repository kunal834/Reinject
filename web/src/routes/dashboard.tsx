    ...
    if (current) {
      ...
      try {
        ...
        if (typeof current.questions === 'string') {
          // just for a s safety
          questionsArr = current.questions.includes('[object Object]')
            ? []
            : JSON.parse(current.questions)
        } else if (Array.isArray(current.questions)) {
          // expected this will run every time
    ...
    const loadResponses = async () => {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/surveys/surveys/${selectedSurveyId}/responses`,
          {
            credentials: 'include',
          },
        )
    ...
    >
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div
          ...
          className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between border border-slate-800 p-6 rounded-none"
        >
          <div>
            ...
            <div className="mt-4 flex flex-wrap items-center gap-2 bg-black/40 p-2 border border-slate-800 max-w-xl rounded-md">
              {PRESET_THEMES.map((theme) => (
                <button
                  ...
                  style={{
                    borderColor: activeTheme.name === theme.name ? theme.accent : 'transparent',
                  }}
        ...
        {viewMode === 'list' ? (
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
            <div className="xl:col-span-2 space-y-4">
              <h2
                ...
                className="text-2xl font-black uppercase tracking-widest font-['VT323',_monospace]"
              >
              ...
              {surveys.length === 0 ? (
                <div
                  style={{ backgroundColor: activeTheme.pane }}
                  className="border border-dashed border-slate-800 p-12 text-center text-slate-500"
                >
                  <p className="font-bold uppercase text-xs text-[#ff007f]">
                    ...
                    <div
                      ...
                      className={`border-4 p-5 transition-all ${
                        selectedSurveyId === survey.id
                          ? 'border-[#00f5ff] border-b-8 shadow-[4px_4px_0px_0px_#008b94]'
                          : 'border-black border-b-8'
                      }`}
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <h3 className="font-black uppercase text-white text-lg">
                            {survey.title}
                          </h3>
                          <p className="text-[10px] text-slate-400 font-mono mt-1 bg-black/40 inline-block px-2 py-0.5 border border-slate-900 rounded-md">
                            ID: {survey.id.slice(0, 18)}...
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="mt-6 flex flex-wrap items-center gap-4 text-xs border-t border-slate-900 pt-4 font-bold">
                        <div className="text-slate-300">
                          <strong className="text-[#00f5ff] bg-black/30 px-1.5 py-0.5 border border-slate-900 mr-1 rounded-md">
                            {survey.questions_count || 0}
                          </strong>{' '}
                          elements
                        </div>
                        <div className="text-slate-300">
                          <strong className="text-[#39ff14] bg-black/30 px-1.5 py-0.5 border border-slate-900 mr-1 rounded-md">
                        ...
                        <div className="ml-auto flex items-center gap-3">
                          <button
                            ...
                            className={`border-2 border-b-4 border-black px-3 py-1.5 text-[11px] font-black uppercase ${copiedId === survey.id ? 'bg-[#39ff14] text-black' : 'bg-slate-950 text-[#00f5ff] border-slate-800'}`}
                          >
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              setSelectedSurveyId(survey.id === selectedSurveyId ? null : survey.id)
                            }
                            className={`border-2 border-b-4 border-black px-3 py-1.5 text-[11px] font-black uppercase ${selectedSurveyId === survey.id ? 'bg-[#ff007f] text-white' : 'bg-[#ffaa00] text-black'}`}
                          >
            ...
            <div className="space-y-6">
              <div
                style={{ backgroundColor: activeTheme.pane }}
                className="border border-slate-800 p-6 min-h-[200px]"
              >
                {!selectedSurveyId ? (
                  <div className="flex h-[140px] flex-col items-center justify-center text-center border-2 border-dashed border-slate-900 rounded-md">
                    <p className="text-[10px] text-slate-500 font-bold uppercase p-4">
                      // Select "Inspect" to view telemetry logs
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <span className="text-[10px] font-bold text-[#ff007f] uppercase tracking-widest bg-[#ff007f]/5 px-2 py-0.5 border border-[#ff007f]/20 shadow-[0_0_10px_rgba(255,0,127,0.05)]">
                      Incoming_Telemetry_Log
                    </span>
                    <div className="space-y-3 max-h-[220px] overflow-y-auto pr-1 custom-scrollbar">
                      {isResponsesLoading ? (
                        <p className="text-xs text-center py-4 animate-pulse text-slate-500">
                          // Interrogating database...
                        </p>
                      ) : responses.length === 0 ? (
                        <p className="text-xs text-center py-4 text-slate-500">
                          // No entries found on this node.
                        </p>
                      ) : (
                        responses.map((res) => (
                          <div
                            key={res.id}
                            className="border border-slate-800 bg-black/40 p-2.5 space-y-2 text-xs rounded-md"
                          >
                            <div className="text-[10px] text-[#00f5ff] font-bold border-b border-slate-900 pb-1">
                              RECORD_NODE_HASH #{res.id.slice(0, 8).toUpperCase()}
                            </div>
                            {Object.entries(res.answers).map(([k, v]) => (
                              <div key={k} className="font-mono text-slate-300">
                                <span className="text-[#ffff55]">{k}:</span> {String(v)}
                              </div>
                            ))}
                          </div>
                        ))
                  ...
                  <div className="relative z-10 bg-[#f8fafc] border border-slate-200 p-4 space-y-3 rounded-md text-slate-900">
                    <div className="flex items-center gap-2 border-b border-slate-200 pb-2">
                      {previewSurvey.parsedBranding?.logoUrl ? (
                        <img
                          src={previewSurvey.parsedBranding.logoUrl}
                          alt="Logo core"
                          className="h-4 max-w-[75px] object-contain"
                          onError={(e) => {
                      ...
                      <h3 className="text-xs font-black uppercase text-slate-950 truncate">
                        {previewSurvey.title}
                      </h3>
                    </div>
                    ...
                    <div className="space-y-3 max-h-[240px] overflow-y-auto pr-1 custom-scrollbar">
                      {previewSurvey.parsedQuestions?.map((q: any, idx: number) => (
                        <div
                          key={q.id}
                          className="bg-white p-3 border border-slate-200 space-y-1.5 rounded-sm shadow-sm"
                        >
                          <p className="text-xs font-bold text-slate-900">
                            {idx + 1}. {q.label}
                          </p>
                          {q.type === 'short_text' && (
                            <div className="h-6 bg-slate-50 border border-slate-200 w-full rounded-sm" />
                          )}
                          ...
                          {q.type === 'multiple_choice' && (
                            <div className="space-y-1">
                              {q.options?.map((o: string) => (
                                <div
                                  key={o}
                                  className="text-[9px] bg-slate-50 border border-slate-200 p-1 text-slate-600 rounded-xs truncate"
                                >
                                  ▪️ {o}
                                </div>
                              ))}
                            ...
                            <div className="flex gap-1">
                              {['1', '2', '3', '4', '5'].map((n) => (
                                <span
                                  key={n}
                                  style={{
                                    borderColor: previewSurvey.parsedBranding?.primaryColor,
                                  }}
                                  className="w-4 h-4 bg-slate-50 border text-[8px] font-black flex items-center justify-center text-slate-500 rounded-sm"
                                >
                                  {n}
                                </span>
                              ))}
                    ...
                    <button
                      ...
                      disabled
                      style={{
                        backgroundColor: previewSurvey.parsedBranding?.primaryColor || '#ff007f',
                      }}
                      className="w-full py-2 rounded-md text-xs font-black uppercase text-white tracking-widest opacity-90 shadow-md"
                    >
                      LIVE_PREVIEW_ACTIVE
                    </button>
                  </div>
          ...
          <div
            style={{ backgroundColor: activeTheme.pane }}
            className="border border-slate-800 p-6 rounded-none"
          >
