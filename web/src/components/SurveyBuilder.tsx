...
const BG_STYLE_PRESETS = [
  {
    id: 'dirt',
    name: '🌌 Neon Cyan Matrix',
    hex: '#111625',
    desc: 'Deep cyberspace baseline node',
  },
  {
    id: 'netherrack',
    name: '🔮 Laser Magenta Layer',
    hex: '#210b14',
    desc: 'Reflective ultra-bright core color',
  },
  {
    id: 'endstone',
    name: '🪐 Orchid Violet Grid',
    hex: '#150a21',
    desc: 'Eerie high-contrast premium workspace',
  },
  {
    id: 'deepslate',
    name: '🐦 Obsidian Slate Hull',
    hex: '#0f172a',
    desc: 'Industrial dark telemetry console',
  },
  {
    id: 'obsidian',
    name: '🕶️ Pure Pitch Void',
    hex: '#070510',
    desc: 'Absolute zero light baseline absorption',
  },
]
...
export function SurveyBuilder({ onCreateSuccess }: SurveyBuilderProps) {
  ...
  const addQuestion = (type: 'short_text' | 'multiple_choice' | 'rating') => {
    ...
    setQuestions([
      ...questions,
      { id: crypto.randomUUID(), type, label: '', options: defaultOptions },
    ])
  }
  ...
  const removeOption = (qId: string, optIdx: number) => {
    setQuestions(
      questions.map((q) =>
        q.id === qId ? { ...q, options: q.options.filter((_, idx) => idx !== optIdx) } : q,
      ),
    )
  }
  ...
  return (
    <form
      ...
      className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start text-[#e2e8f0] font-['Share_Tech_Mono',_monospace]"
    >
      ...
      <div className="lg:col-span-2 space-y-6 bg-[#f8fafc] border-2 border-slate-300 p-8 rounded-none shadow-2xl text-slate-900">
          ...
          {questions.length === 0 ? (
            ...
            <div className="space-y-6">
              {questions.map((q, idx) => (
                <div
                  key={q.id}
                  className="group bg-white border border-slate-200 p-5 space-y-4 relative shadow-sm"
                >
                  <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-wider text-slate-400">
                    <span>
                      Field Element #{idx + 1} — {q.type.replace('_', ' ')}
                    </span>
                    <div className="flex gap-3 items-center opacity-60 group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="text-slate-500 disabled:opacity-20 hover:text-indigo-600 font-bold cursor-pointer"
                      >
                        ▲ Move Up
                      </button>
                      <button
                  ...
                  {q.type === 'multiple_choice' && (
                    <div className="space-y-2 pl-4 pt-1">
                        ...
                        <div key={oIdx} className="flex gap-2 items-center">
                          ...
                          {q.options.length > 2 && (
                            <button
                              ...
                              className="text-xs text-red-500 font-bold px-1.5 hover:text-red-700"
                            >
                              ✕
                            </button>
                          )}
                      ...
                      <button
                        ...
                        className="text-[10px] font-black text-indigo-600 uppercase mt-2 block hover:underline"
                      >
                        + Append Choice Row
                      </button>
                    </div>
        ...
        <div className="border-t border-slate-200 pt-6 grid grid-cols-1 sm:grid-cols-3 gap-3">
          <button
            ...
            className="bg-slate-100 border border-slate-300 text-slate-700 hover:bg-slate-200 py-2.5 text-xs font-bold transition-all cursor-pointer text-center"
          >
            📝 Add Text Input
          </button>
          <button
      ...
      <div className="space-y-6">
        <div className="border border-slate-800 bg-[#121626]/80 p-6 space-y-6 rounded-none relative before:absolute before:inset-0 before:border before:border-dashed before:border-slate-900 before:pointer-events-none">
          <h2 className="text-xl font-black uppercase text-[#ff007f] border-b border-slate-900 pb-3 font-['VT323',_monospace] tracking-wider">
            // 03. SKIN_BRAND_SCHEMA_TOKENS
          </h2>

          <div className="space-y-3">
            <label className="block text-xs font-bold uppercase tracking-widest text-[#00f5ff]">
                  ...
                  return (
                    <button
                      ...
                      className={`text-left p-2 border transition-all cursor-pointer rounded-md ${
                        isCurrent
                          ? 'bg-slate-950 border-[#00f5ff] text-white font-black shadow-[0_0_10px_rgba(0,245,255,0.1)]'
                    ...
                    >
                      ...
                      <div className="text-[9px] opacity-60 font-mono font-normal mt-0.5 tracking-tight">
                        {preset.desc}
                      </div>
                    </button>
                  )
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="h-9 w-12 border border-black cursor-pointer bg-transparent rounded-md"
                />
                <input
                  type="text"
                  value={customBgColor}
                  onChange={(e) => setCustomBgColor(e.target.value)}
                  className="w-full bg-[#070b12] border border-black px-3 text-xs font-mono font-bold text-[#00f5ff] uppercase rounded-md focus:outline-none"
                />
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            <label className="block text-xs font-bold uppercase tracking-widest text-slate-400">
              Primary Core Button Color Accent
            </label>
            <div className="flex gap-2">
              <input
                type="color"
          ...
          <div className="space-y-2">
            <label className="block text-xs font-bold uppercase tracking-widest text-slate-400">
              Branded Enterprise Logo Endpoint Asset Link
            </label>
            <input
              type="url"
              placeholder="https://company.com/branding_logo.png"
              value={logoUrl}
              onChange={(e) => setLogoUrl(e.target.value)}
              className="w-full bg-[#070b12] border border-black p-2.5 text-xs font-bold text-[#00f5ff] placeholder-[#ff007f]/20 focus:outline-none focus:border-[#ff007f] transition-colors rounded-md"
            />
          </div>
        </div>
          ...
          >
            ...
            <div className="relative z-10 bg-[#f8fafc] border border-slate-200 p-4 space-y-3 rounded-md text-slate-900">
              <div className="flex items-center gap-2 border-b border-slate-200 pb-2">
                ...
                <span className="text-[10px] font-black text-slate-950 uppercase tracking-widest truncate max-w-[140px] font-['Share_Tech_Mono',_monospace]">
                  {localLiveMock.title}
                </span>
              </div>
              ...
              <div className="space-y-3 max-h-[180px] overflow-y-auto pr-1 custom-scrollbar">
                {localLiveMock.questions.length === 0 ? (
                  ...
                  localLiveMock.questions.map((q: any, i: number) => (
                    <div
                      key={q.id}
                      className="bg-white p-2 border border-slate-200 text-[10px] space-y-2 rounded-sm"
                    >
                      <span className="text-slate-900 font-bold block">
                        {i + 1}. {q.label || 'Untitled Field'}
                      </span>
                      {q.type === 'short_text' && (
                        <div className="h-5 bg-slate-50 border border-slate-200 w-full" />
                      )}
                      ...
                      {q.type === 'multiple_choice' && (
                        <div className="space-y-1">
                          {q.options?.map((optionString: string) => (
                            <div
                              key={optionString}
                              className="text-[8px] text-slate-500 font-mono bg-slate-50 px-1.5 py-0.5 border border-slate-200 truncate"
                            >
                              ▪️ {optionString}
                            </div>
                          ))}
                      ...
                      {q.type === 'rating' && (
                        <div className="flex gap-1">
                          {['1', '2', '3', '4', '5'].map((n) => (
                            <span
                              key={n}
                              style={{ borderColor: localLiveMock.primaryColor }}
                              className="w-4 h-4 bg-slate-50 text-center text-[8px] text-slate-400 border flex items-center justify-center rounded-xs"
                            >
                              {n}
                            </span>
                          ))}
              ...
              <button
                type="button"
                disabled
                style={{ backgroundColor: localLiveMock.primaryColor }}
                className="w-full text-center text-white text-[10px] font-black uppercase py-2 border border-black opacity-90 tracking-widest"
              >
                LIVE_PREVIEW_ACTIVE
              </button>
            </div>
